...
def assumption_analysis(base_failure, new_failure, verbose=False, min_runtime=0):
    ...
    for line in open(log_file):
        m = re.search(r"Predicting next suspect (\d+)", line)
        if m:
            s = int(m.group(1))
            if s != 0:
...
def main(args):
    ...
    if "Guidance method = 2" in log:
        base_points = []
        new_points = []
        recall_auc_improvementz = []
        
        for failure in all_failurez: 
            recall_auc_improvement = assumption_analysis(failure, failure+args.new_suffix, 
        ...
        print "Mean recall auc improvement: %.3f" %(np.mean(recall_auc_improvementz))
        print "Median recall auc improvement: %.3f" %(np.median(recall_auc_improvementz))
        
    elif "Guidance method = 0" not in log:
        recalls = []
        speedups = []
        mem_reductions = []
        block_recalls = []
        ...
        for failure in all_failurez: 
            recall, speedup, mem_reduce, block_recall, block_acc = blocking_analysis(failure, failure+args.new_suffix, 
                verbose=args.verbose, min_runtime = args.min_runtime)
